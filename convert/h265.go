...
func Convert2H265(in GetFileInfo.Info, threads string) {
	...
	if err != nil {
		slog.Warn("cmd.StdoutPipe产生错误", slog.Any("产生错误", err))
		return
	}
	if err = cmd.Start(); err != nil {
		slog.Warn("cmd.Run产生的错误", slog.Any("产生错误", err))
		return
	}
	for {
		tmp := make([]byte, 1024)
	...
	if err = cmd.Wait(); err != nil {
		slog.Warn("命令执行中有错误产生", slog.Any("产生错误", err))
		return
	}
...
func ConvertOne(src, dst, threads string) {
	...
	if err = cmd.Wait(); err != nil {
		slog.Warn("命令执行出错", slog.Any("产生错误", err))
		return
	}
	//log.Debug.Printf("完成当前文件的处理:源文件是%s\t目标文件是%s\n", in, file)
